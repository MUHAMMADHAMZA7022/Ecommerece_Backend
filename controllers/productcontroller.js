...
exports.productreview = catchasyncerror(async (req, res, next) => {
    ...
    const isreview = await rproduct.reviews.find((rev) => rev.user.toString() === req.user._id.toString())
    if (isreview) {
...
exports.deletereview = catchasyncerror(async (req, res, next) => {
    ...
    const reviews = dproduct.reviews.filter(rev => rev._id.toString() !== req.query.id.toString());
    let avg = 0;
    reviews.forEach(rev => {
        avg = avg + rev.rating
    })
