...
process.on("unhandledRejection", err => { //ye build in function ha sb kuch 
    console.log("Shutting Down The Server due to Unhandled promise Rejection")
    console.log(`Error,${err.message}`) //ye message aye hamei k kia error ha
    server.close(() => { //ye hum ny jldi sy server close kr diya or bd mein call back function dy k osy exit kr diya is process sy
        process.exit(1);
    })
